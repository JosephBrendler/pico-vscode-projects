...
//    pio->sm[0].clkdiv = (uint32_t) (2.5f * (1 << 8));
    pio->sm[0].clkdiv = (uint32_t) (1 << 16);
/// \end::clock_divider[]
    ...
    // as opposed to e.g. the processors. // Correction for x4's rp2040, wherein
    // GPIO0 is not available on the pin header, use GPIO2 instead
    /*
    pio->sm[0].pinctrl =
    ...
    */
    pio->sm[0].pinctrl =
    (1 << PIO_SM0_PINCTRL_SET_COUNT_LSB) |
    (2 * (1 << PIO_SM0_PINCTRL_SET_BASE_LSB));
